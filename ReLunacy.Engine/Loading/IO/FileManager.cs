...
public class FileManager : IDisposable
{
    ...
    private Stream? OpenFromFolder(string name)
    {
        ...
        if (File.Exists(path)) return File.Open(path, FileMode.Open, FileAccess.Read);
    ...
    {
        if (archive.Manifest.ContainsKey(name)) return name;
        return archive.Paths.FirstOrDefault(p => p.EndsWith("/" + name, StringComparison.OrdinalIgnoreCase));
    }

    public void Dispose()
    {
        foreach (var file in igfiles.Values) file?.Dispose();
        igfiles.Clear();
        foreach (var stream in rawfiles.Values) stream?.Dispose();
        rawfiles.Clear();
